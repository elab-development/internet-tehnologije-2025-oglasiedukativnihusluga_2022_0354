...
// Prvo proverimo da li već postoje predmeti
const existingPredmeti = await db.select().from(predmet);

let predmeti;
if (existingPredmeti.length === 0) {
  predmeti = await db.insert(predmet).values([
    {nazivPredmeta:"Matematika"},
    {nazivPredmeta:"Fizika"},
    {nazivPredmeta:"Engleski jezik"}
  ]).returning();
  console.log("✅ Predmeti dodati");
} else {
  predmeti = existingPredmeti;
  console.log("⚠️ Predmeti već postoje, koristim postojeće");
}

// Proverimo da li već postoje korisnici
const existingUsers = await db.select().from(korisnik);
if (existingUsers.length > 0) {
  console.log("⚠️ Korisnici već postoje, završavam seed");
  return;
}
