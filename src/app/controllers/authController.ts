...
import { db } from "@/db";
import { korisnik, tutor } from "@/db/schema";
...
export const authController = {
  ...
  let finalRole: "ADMIN" | "KORISNIK" | "TUTOR" = uloga || "KORISNIK";
  ...
  // Ako se registruje kao TUTOR, automatski kreiraj tutor profil
  if (finalRole === "TUTOR") {
    ...
    console.log("✅ Tutor profil automatski kreiran za korisnika:", korisnikId);
  }
  ...
  async me(token?: string) {
    try {
      ...
      // Učitaj kompletan user profil iz baze
      const [userFromDb] = await db
        ...
        .from(korisnik)
        .where(eq(korisnik.id, tokenData.id));
