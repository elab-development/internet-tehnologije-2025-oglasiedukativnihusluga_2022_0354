...
export const oglasController = {
  ...
  // DELETE
  async delete(idOglas: number, userId: number, isAdmin: boolean = false) {
    const [existing] = await db
      .select()
      .from(oglas)
      .where(eq(oglas.idOglas, idOglas));
    ...
    // Admin može brisati sve, tutor samo svoje
    if (!isAdmin && existing.tutorId !== userId) {
      throw new Error("Nije dozvoljeno brisati tuđi oglas");
    }
